...
{
    if(ENCODER_ENABLED_Z_DEFAULT == 0) return;    //2018.03.02-FBTUG
  
    Serial.print(COMM_REPORT_ENCODER_SCALED);   //R84-Report encoder position scaled
    ...
    //------------------------------------------------------------
    Serial.print(COMM_REPORT_ENCODER_RAW);    //-R85-Report encoder position raw
...
unsigned int commandSpeed[3];
   ...
   //Serial.print(stepsPerMm[1]); Serial.print(" , "); Serial.println(stepsPerMm[2]);   //-2018.02.24
    ...
    //Serial.print("x.y.z Speed :"); Serial.print(xMaxSpd); Serial.print(" , ");            //-2018.03.02
    //Serial.print(yMaxSpd); Serial.print(" , "); Serial.println(zMaxSpd);    //-2018.03.02
    
    //Serial.print("Speed :"); Serial.print(commandSpeed[0]); Serial.print(" , ");            //-2018.03.02
    //Serial.print(commandSpeed[1]); Serial.print(" , "); Serial.println(commandSpeed[2]);    //-2018.03.02
    ...
    //Command.cpp 已改正如下,應該也可以不用如上這麼寫了
    ...
    while ((axisActive[0] || axisActive[1] || axisActive[2]) && !EmergencyStop){   //-2018.02.24  
        ...
        */
        ...
        if (Serial.available() > 0){
            ...
            if (incomingByte == 69 || incomingByte == 101){   //E=69, e=101
            ...
            if (incomingByte == 83 || incomingByte == 115){   //-S=83, s=115 Stop Axis motion-2018.03.03        
                axisX.deactivateAxis(); axisY.deactivateAxis(); axisZ.deactivateAxis();
                axisActive[0] = false; axisActive[1] = false; axisActive[2] = false;            
            }
        }
  ...
  while (!movementDone && error == 0){
    ...
    if ((!calibAxis->endStopMin() && !calibAxis->endStopMax()) && !movementDone && (*missedSteps < *missedStepsMax)){
      ...
      if (stepsCount % (speedHome[axis] * 3) == 0){
        ...
        //pMotionStatus[0] = 4;  //-R4-2018.05.05-CGH
        CurrentState::getInstance()->setMotionStatus(4);    //-2018.05.05
...
void StepperControl::disableMotors()
{
    ...
    if (motorKeepActive[0] == 0) axisX.disableMotor();  //-ServoOFF
    if (motorKeepActive[1] == 0) axisY.disableMotor();  //-ServoOFF
    if (motorKeepActive[2] == 0) axisZ.disableMotor();  //-ServoOFF
  
    delay(100);
}

//------------------------------------------------------------------------------------------
void StepperControl::ServoON(int vAxis, int vOnOff)   //-2018.03.02-ServoOn/Off
{
    motorKeepActive[vAxis] = vOnOff;
...
//------------------------------------------------------------------------------------------
void StepperControl::Stop_Axis(int vAxis)   //-2018.03.03
{
    if(vAxis == 0) axisX.deactivateAxis();
    if(vAxis == 1) axisY.deactivateAxis();
    if(vAxis == 2) axisZ.deactivateAxis();        
}
...
{
    if (axisX.endStopsReached() || axisY.endStopsReached() || axisZ.endStopsReached()){
        return true;
    }
    return false;
}
