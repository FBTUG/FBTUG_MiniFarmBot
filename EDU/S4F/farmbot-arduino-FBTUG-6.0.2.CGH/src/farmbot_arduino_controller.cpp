...
//extern unsigned int pMotionStates[3];   //-2018.05.05
...
void setup()
{
    //-------------------------------------------------------------- 
      ...
      pinMode(UTM_D, INPUT);      //-2018.06.03-CGH,Analog讀值才正常(Capacitive Soil Moisture Sensor)
      pinMode(UTM_E, INPUT);      //-2018.06.03-CGH,(TEMP Sensor)
      pinMode(UTM_F, INPUT);      //-2018.06.03-CGH
      pinMode(UTM_G, INPUT);      //-2018.06.03-CGH
      pinMode(UTM_H, INPUT);      //-2018.06.03-CGH
      //pinMode(UTM_I, OUTPUT);   //D4-SERVO_0
      //pinMode(UTM_J, OUTPUT);   //D5-SERVO_1
      //pinMode(UTM_K, OUTPUT);   //D6-SERVO_2
      //UTM_L - D9-12V
      ...
      pinMode(SERVO_0_PIN , OUTPUT);    //-2018.06.03-UTM_I
      pinMode(SERVO_1_PIN , OUTPUT);    //-2018.06.03-UTM_J
    #endif
    ...
    //--------------------------------------------------------------
      ...
      pinMode(FAN_PIN, OUTPUT);   // Changed by FBTUG (mini farmbot)-2018.02.14
    ...
    //  Serial2.begin(9600);  //D16-Tx2, D17-Rx2->BT-HC06-2018.02.25-CGH
    //#endif  
    
    //#ifdef ESP_Serial2_Use
    //  Serial2.begin(115200);  //D16-Tx2, D17-Rx2->ESP-01-2018.08.08-CGH
    //#endif    
    ...
    #ifdef Serial2_Use    //-2018.08.15 
      Serial2.print("Serial2 BaudRate = "); Serial2.println(lBaudRate);
    #endif    
    ...
    if((ParameterList::getInstance()->getValue(PARAM_CONFIG_OK) == 1) &&
        ...
        #ifdef Serial2_Use
          Serial2.print("R99 HOME Z ON STARTUP\r\n");   //-2018.02.25-CGH
        #endif  
    ...
    if((ParameterList::getInstance()->getValue(PARAM_CONFIG_OK) == 1) &&
        ...
        #ifdef Serial2_Use
          Serial2.print("R99 HOME Y ON STARTUP\r\n");   //-2018.02.25-CGH
        #endif
    ...
    if((ParameterList::getInstance()->getValue(PARAM_CONFIG_OK) == 1) &&
        ...
        #ifdef Serial2_Use
          Serial2.print("R99 HOME X ON STARTUP\r\n");   //-2018.02.25-CGH
        #endif  
    ...
    #ifdef Serial2_Use
      Serial2.print("R99 ARDUINO STARTUP COMPLETE\r\n");   //-2018.02.25-CGH
    #endif  
}
...
void loop()
{
    ...
    if(Serial.available()){
        ...
        fSerial_Data_Proc();  //-2018.02.25-CGH
    }
    ...
    if(previousEmergencyStop == false && CurrentState::getInstance()->isEmergencyStop()){
        ...
        if (debugMessages){
            Serial.print(COMM_REPORT_COMMENT); Serial.print(" Going to safe state");    //-R89    
            #ifdef Serial2_Use
              Serial2.print(COMM_REPORT_COMMENT); Serial2.print(" Going to safe state");  //-2018.02.25-CGH
            #endif   
    ...
    if (lastParamChangeNr != ParameterList::getInstance()->paramChangeNumber()){
        ...
        if (debugMessages){
            Serial.print(COMM_REPORT_COMMENT); Serial.print(" loading parameters");   //-R89
            #ifdef Serial2_Use
              Serial2.print(COMM_REPORT_COMMENT); Serial2.print(" loading parameters");  //-2018.02.25-CGH
            #endif    
            CurrentState::getInstance()->printQAndNewLine();
        }
    ...
    if (currentTime < lastAction){
        ...
        if ((currentTime - lastAction) > 5000){   //-5's
            ...
            if(CurrentState::getInstance()->isEmergencyStop()) {
                ...
                if (debugMessages){
                    Serial.print(COMM_REPORT_COMMENT); Serial.print(" Emergency stop engaged");   //-R89
                    #ifdef Serial2_Use
                      Serial2.print(COMM_REPORT_COMMENT); Serial2.print(" Emergency stop engaged");   //-2018.02.25-CGH          
                    #endif 
                    CurrentState::getInstance()->printQAndNewLine();
                }
            }
            else{
                //pMotionStatus[0] = 0;  //-R0-2018.05.05-CGH
                CurrentState::getInstance()->setMotionStatus(0);    //-2018.05.05
                  
                Serial.print(COMM_REPORT_CMD_IDLE);        
                #ifdef Serial2_Use
                  Serial2.print(COMM_REPORT_CMD_IDLE);    //-2018.02.25-CGH
                #endif
            ...
            if (debugMessages){
                Serial.print(COMM_REPORT_COMMENT); Serial.print(" MEM "); Serial.print(freeMemory());   //-R89
                #ifdef Serial2_Use
                  Serial2.print(COMM_REPORT_COMMENT); Serial2.print(" MEM "); Serial2.print(freeMemory());    //-2018.02.25-CGH
                #endif
                CurrentState::getInstance()->printQAndNewLine();
        
                Serial.print(COMM_REPORT_COMMENT); Serial.print(" IND DUR "); Serial.print(interruptDuration);    //-R89
                Serial.print(" MAX "); Serial.print(interruptDurationMax);
                #ifdef Serial2_Use
                  Serial2.print(COMM_REPORT_COMMENT); Serial2.print(" IND DUR "); Serial2.print(interruptDuration);   //-2018.02.25-CGH
                  Serial2.print(" MAX "); Serial2.print(interruptDurationMax);  //-2018.02.25-CGH                                        
                #endif               
                CurrentState::getInstance()->printQAndNewLine();
            ...
            if (ParameterList::getInstance()->getValue(PARAM_CONFIG_OK) != 1){
                Serial.println(COMM_REPORT_NO_CONFIG);    //-R88 - 2018.08.15+ln
                #ifdef Serial2_Use
                  Serial2.println(COMM_REPORT_NO_CONFIG);   //-2018.02.25-CGH - 2018.08.15+ln
                #endif         
            }
...
//------------------------------------------------------------------------------------------
void fSerial_Data_Proc()  //-2018.02.25-CGH
{
    ...
    if (incomingChar == '\n' || incomingCommandPointer >= INCOMING_CMD_BUF_SIZE){
        ...
        if (incomingCommandPointer > 1){
            ...
            Serial.print(" "); Serial.print("*"); Serial.print(commandChar); Serial.print("*"); Serial.print("\r\n");  
            #ifdef Serial2_Use
              Serial2.print(COMM_REPORT_CMD_ECHO);  //-2018.02.25-CGH
              Serial2.print(" "); Serial2.print("*"); Serial2.print(commandChar); Serial2.print("*"); Serial2.print("\r\n");  //-2018.02.25-CGH
            #endif
