...
//extern const int MOVEMENT_SPD_X_DEFAULT;   //-2018.03.04-FBTUG-3500-不能這樣用,會錯誤
//extern const int MOVEMENT_SPD_Y_DEFAULT;   //-2018.03.04-FBTUG-3500-不能這樣用,會錯誤
//extern const int MOVEMENT_SPD_Z_DEFAULT;   //-2018.03.04-FBTUG-3500-不能這樣用,會錯誤
...
const char numberWatercapacityCode = 'N';  //Water capacity-FBTUG-Joy-2018.06.03

CommandCodeEnum commandCodeEnum = CODE_UNDEFINED;

double axisValue[3] = {0.0, 0.0, 0.0};        //-2018.03.02-FBTUG-放外面,才不會一直被清為0
long axisSpeedValue[3] = {0, 0, 0};           //-2018.03.02-FBTUG-放外面,才不會一直被清為0
...
bool invalidCommand = false;
    ...
    //Serial.println("Command::Command");   //-2018.03.02
  
    //if(charPointer[0] == 'G' || charPointer[0] == 'F'){
    if(charPointer[0] == 'G' || charPointer[0] == 'F' || charPointer[0] == 'T'){    //-2018.06.03-FBTUG
...
CommandCodeEnum Command::getGCodeEnum(char *code)
{
    ...
    if (strcmp(code, "G0") == 0 || strcmp(code, "G00") == 0){
        ...
        //Serial.print("axisSpeedValue = "); Serial.println(axisSpeedValue[0]);   //-2018.03.02
        
        //-當axisSpeedValue為0時,給 MOVEMENT_SPD_X_DEFAULT 預設速度
        if(axisSpeedValue[0] == 0) axisSpeedValue[0] = MOVEMENT_SPD_X_DEFAULT;    //-2018.03.02-FBTUG        
        if(axisSpeedValue[1] == 0) axisSpeedValue[1] = MOVEMENT_SPD_Y_DEFAULT;    //-2018.03.02-FBTUG   
        if(axisSpeedValue[2] == 0) axisSpeedValue[2] = MOVEMENT_SPD_Z_DEFAULT;    //-2018.03.02-FBTUG   
        
        //Serial.print("New axisSpeedValue = "); Serial.println(axisSpeedValue[0]); 
...
void Command::getParameter(char *charPointer)
{
    ...
    if (charPointer[0] == axisSpeedCodes[0]){   //-A.x
        axisSpeedValue[0] = atof(charPointer + 1);
        //Serial.print("A.x = "); Serial.println(axisSpeedValue[0]);   //-2018.03.02
    }
  
    if (charPointer[0] == axisSpeedCodes[1]){   //-B.y
        axisSpeedValue[1] = atof(charPointer + 1);
        //Serial.print("B.y = "); Serial.println(axisSpeedValue[1]);   //-2018.03.02
    }
  
    if (charPointer[0] == axisSpeedCodes[2]){   //-C.z
        axisSpeedValue[2] = atof(charPointer + 1);
        //Serial.print("C.z = "); Serial.println(axisSpeedValue[2]);   //-2018.03.02
    }
  
    if (charPointer[0] == speedCode){           //S
        speedValue = atof(charPointer + 1);
        
        //Serial.print("speedValue = "); Serial.println(speedValue);   //-2018.03.02
        
        //G00 S沒作用, 改一下,更新到 A-xSpeed, B-ySpeed, C-zSpeed 內
        axisSpeedValue[0] = speedValue;    //-A.x-2018.03.02-FBTUG
        axisSpeedValue[1] = speedValue;    //-B.y-2018.03.02-FBTUG
        axisSpeedValue[2] = speedValue;    //-C.z-2018.03.02-FBTUG

        //Serial.print("axisSpeedValue[0] = "); Serial.println(axisSpeedValue[0]);   //-2018.03.02
    }
    ...
    if (charPointer[0] == numberWatercapacityCode){   //-FBTUG-Joy-2018.06.03
        numberWatercapacity = atof(charPointer + 1);
    }    
}
...
void Command::print()
{
    ...
    Serial.print(", N: ");                //-FBTUG-Joy-2018.06.03        
    Serial.print(numberWatercapacity);    //-FBTUG-Joy-2018.06.03
...
//------------------------------------------------------------------------------------------
long Command::getS()  //-2018.03.02-FBTUG
{
    return speedValue;
}
