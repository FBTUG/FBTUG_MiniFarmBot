...
{
  Serial.print("R99"); Serial.print(" cur loc = "); Serial.print(coordCurrentPoint);
  //Serial.print(" enc loc = "); Serial.print(coordEncoderPoint);
  //Serial.print(" cons steps missed = "); Serial.print(label); Serial.print(consMissedSteps);
  Serial.print("\r\n");

  //-2018.02.25-CGH
  #ifdef BT_Serial2_Use
    Serial2.print("R99"); Serial2.print(" cur loc = "); Serial2.print(coordCurrentPoint);
    //Serial2.print(" enc loc = "); Serial2.print(coordEncoderPoint);
    //Serial2.print(" cons steps missed = "); Serial2.print(label); Serial2.print(consMissedSteps);
    Serial2.print("\r\n");  
  #endif   
}
...
long endPos;
  ...
  */
  ...
  if (debugPrint && (millis() - lastCalcLog > 1000)){
    ...
    Serial.print("R99");
    Serial.print(" sta "); Serial.print(staPos);
    Serial.print(" cur "); Serial.print(curPos);
    Serial.print(" end "); Serial.print(endPos);
    Serial.print(" half "); Serial.print(halfway);
    Serial.print(" len "); Serial.print(stepsAccDec);
    Serial.print(" min "); Serial.print(minSpeed);
    Serial.print(" max "); Serial.print(maxSpeed);
    Serial.print(" spd "); Serial.print(" "); Serial.print(newSpeed);
    Serial.print("\r\n");

    //-2018.02.25-CGH
    #ifdef BT_Serial2_Use
      Serial2.print("R99");
      Serial2.print(" sta "); Serial2.print(staPos);
      Serial2.print(" cur "); Serial2.print(curPos);
      Serial2.print(" end "); Serial2.print(endPos);
      Serial2.print(" half "); Serial2.print(halfway);
      Serial2.print(" len "); Serial2.print(stepsAccDec);
      Serial2.print(" min "); Serial2.print(minSpeed);
      Serial2.print(" max "); Serial2.print(maxSpeed);
      Serial2.print(" spd "); Serial2.print(" "); Serial2.print(newSpeed);
      Serial2.print("\r\n");    
    #endif  
  }
...
{
    checkAxisDirection();
    //+ movementUp = true; - movementUp = false;
  
    //-G28, F11, F12, F13 coordHomeAxis = true; 往0位置, 直到撞極限才停
    //if(coordHomeAxis) Serial.println("coordHomeAxis = true");   //-2018.02.24
        
    // Handle movement if destination is not already reached or surpassed
    if(((coordDestinationPoint > coordSourcePoint && coordCurrentPoint < coordDestinationPoint) ||
        (coordDestinationPoint < coordSourcePoint && coordCurrentPoint > coordDestinationPoint) ||
         coordHomeAxis) && axisActive){
  
        // home or destination not reached, keep moving - Home點或目的地未到達, 繼續移動
        // If end stop reached or the encoder doesn't move anymore, stop moving motor, otherwise set the timing for the next step
        // 如果到達極限SNR停止或編碼器不再移動，停止移動電機，否則請設置下一步的時間
        //-!endStopAxisReached(false) 不是-極限, 
        //if ((coordHomeAxis && !endStopAxisReached(false)) || (!coordHomeAxis && !endStopAxisReached(!movementToHome))){   //-2018.02.07          
        if(!endStopAxisReached(true) && !endStopAxisReached(false)){     //-未撞到 +- 極限即可繼續變速運動-2018.02.07-CGH             
            // Get the axis speed, in steps per second
            axisSpeed = calculateSpeed(coordSourcePoint, coordCurrentPoint, coordDestinationPoint, motorSpeedMin, motorSpeedMax, motorStepsAcc);
            ...
            //axisActive = false;   //-到+極限要能往-向跑 - 2018.02.24-CGH
            if(endStopAxisReached(true) && movementUp){
                axisActive = false;    //+極限 +方向 2018.02.24-CGH
                Serial.println("checkMovement : +極限 +方向 axisActive = false");
                #ifdef BT_Serial2_Use
                  Serial2.print("checkMovement : +極限 +方向 axisActive = false");    //-2018.02.28-CGH
                #endif                      
            }
            if(endStopAxisReached(false) && !movementUp){
                axisActive = false;  //-極限 -方向 2018.02.24-CGH
                Serial.println("checkMovement : -極限 -方向 axisActive = false");     
                #ifdef BT_Serial2_Use
                  Serial2.print("checkMovement : -極限 -方向 axisActive = false");    //-2018.02.28-CGH
        ...
        // Destination or home reached. Deactivate the axis. 到達 目的地或Home點。 取消激活軸
        axisActive = false;
    }
  
    // If end stop for home is active, set the position to zero
    if (endStopAxisReached(false)){
        coordCurrentPoint = 0;    //-有歸零動作, 再設為0比較OK-2018.02.24-CGH
    }
}
...
bool EndStopNC;   //-FBTUG-v4.0.0-2018.02.24
