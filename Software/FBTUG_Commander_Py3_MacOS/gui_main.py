...
class App:
    ...
    def __init__(self,root):
        strFont= 'Arial'
        myfont14 = tkinter.font.Font(family=strFont, size=14, weight= tkinter.font.BOLD)
        myfont12 = tkinter.font.Font(family=strFont, size=12)#, weight= tkFont.BOLD)
        myfont12_Bold = tkinter.font.Font(family=strFont, size=12, weight= tkinter.font.BOLD)
        myfont10 = tkinter.font.Font(family=strFont, size=10)
        myfont8 = tkinter.font.Font(family=strFont, size=8, weight= tkinter.font.BOLD)
        self.bgGreen= '#007700'
        ...
        self.tabbox = tkinter.ttk.Notebook(self.root, width=Left_width, height=Left_height)
    ...
    def set_Motor(self):
        if self.ArdMntr.connect:
            ...
            tkinter.messagebox.showerror("Error", "Arduino connection refused!\n Please check its connection.")
    ...
    def btn_choosescript_click(self):
        str_scriptPath = tkinter.filedialog.askopenfilename(title = "Select file",filetypes = (("all files","*.*"),("Text File", "*.txt"),("jpeg files","*.jpg")))
    ...
    def btn_loadscript_click(self):
        ...
        if utils_tool.check_file(tmpPath):
            ...
            tkinter.messagebox.showerror("Error", "'%s' dost not exist !" % tmpPath)
        '''
            ...
            if self.ArdMntr.connect:
                try:
                    ...
                    if self.scan_X[0]+self.scan_X[1]*self.scan_X[2]<self.limit[0] | self.scan_Y[0]+self.scan_Y[1]*self.scan_Y[2]<self.limit[1]:
                        ...
                        tkinter.messagebox.showerror("Error", "The scanning of X should be in [0~{0}]\nThe range of Y should be in [0~{1}]".format(self.limit[0],self.limit[1]))
                except:
                    tkinter.messagebox.showerror('Error', 'Please enter nubmer')
            else:
                tkinter.messagebox.showerror("Error", "Arduino connection refused!")
    ...
    def btn_loadImg_click(self):
        str_imagePath = tkinter.filedialog.askopenfilename(title = "Select image",filetypes = (("jpeg files","*.jpg"), ("png files","*.png"), ("tif files","*.tif"),("all files","*.*")))
        print('>>>> ', str_imagePath)
        if str_imagePath !="":
            ...
            if img is not False:
                ...
                tkinter.messagebox.showerror('Image does not exist', 'The image\n{0}\n does not exist. Please check the path again')
        ...
        if self.ArdMntr.connect:
            try:
                ...
                if (Target_X>=0) & (Target_X<=self.limit[0]) & (Target_Y>=0) & (Target_Y<=self.limit[1]):
                    ...
                    tkinter.messagebox.showerror("Error", "The range of X should be in [0~{0}]\nThe range of Y should be in [0~{1}]".format(self.limit[0],self.limit[1]))
...
root = tkinter.Tk()
root.title("[FBTUG] offline Farmbot GUI for development based on Python3")
