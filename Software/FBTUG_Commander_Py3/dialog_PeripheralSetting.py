...
class PeripheralSetting(tkinter.simpledialog.Dialog):
    # ########################################
    def __init__(self, master, arg_PinList=[('',0)]):
        print('init')
        strFont= 'Arial'
        self.__myfont12 = tkinter.font.Font(family=strFont, size=12)
        self.__myfont12_Bold = tkinter.font.Font(family=strFont, size=12, weight= tkinter.font.BOLD)
        self.__myfont10 = tkinter.font.Font(family=strFont, size=10)
        self.__myfont10_Bold = tkinter.font.Font(family=strFont, size=10, weight= tkinter.font.BOLD)
        self.__PinList= arg_PinList
        self.__MaxRow= 7
        self.__CurrentRow= len(arg_PinList)
        self.__CurGridRow= self.__CurrentRow
        self.__NumberList= list(range(0, self.__MaxRow+1))
        self.__entries_Func= [0]
        self.__entries_PinNumb= [0]
        self.__btns_clear=[0]
        #self.master= master
        tkinter.simpledialog.Dialog.__init__(self, master, "Peripherals")
    # ########################################
